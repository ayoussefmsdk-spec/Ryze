...
(function (R) {
  ...
  function tabCures(p, pr) {
    ...
    return `<section class="card"><div class="card-head"><div><h2>Cures — ${esc(pr?.dci)}</h2><div class="sub">${p.cures.filter(c => c.statut === 'realisee').length} réalisée(s) · ${p.cures.filter(c => c.statut === 'prevue').length} planifiée(s) · horizon ${R.fmtDate(p.cures[p.cures.length - 1]?.datePrevue)}</div></div>
      ...
      ${p.cures.map(c => `<tr class="${c.statut === 'realisee' ? 'done' : ''}${c.datePrevue === t && c.statut === 'prevue' ? ' today' : ''}">
        ...
        <td>${c.voie !== 'IV' ? '<span class="muted">—</span>' : c.validationPharma ? `<span class="badge good" title="${esc(R.userName(c.validationPharma.par))} le ${R.fmtDate(c.validationPharma.date)}"><i class="dot"></i>Validée</span>` : '<span class="muted">—</span>'}</td>
        ...
        <td class="actions">${c.statut === 'realisee' ? `<button class="btn sm ghost" data-action="cureDetail" data-pid="${p.id}" data-n="${c.n}">Détails</button>` : ''}
          ...
          ${(c.statut === 'prevue' || c.statut === 'reportee') && w ? `<button class="btn sm primary" data-action="cureEnregistrer" data-pid="${p.id}" data-n="${c.n}">Marquer réalisée</button>` : ''}
  ...
  R.pages.protocoles = {
    render() {
      ...
      return `<div class="page-head"><div><h1>Protocoles thérapeutiques</h1><p>${S.protocoles.length} protocoles · ce sont ceux proposés au médecin lors de la création d’un dossier</p></div>
  ...
  Object.assign(R.actions, {
    ...
    protoEditer(el) {
      ...
      R.modal({ title: pr.id ? `Modifier le protocole — ${esc(pr.dci)}` : 'Nouveau protocole', form: 'protoSave', wide: true, body: `<input type="hidden" name="id" value="${esc(pr.id)}">
        ...
        <div class="field"><label>Article de stock (entretien, si différent)</label><select name="articleEntretienId"><option value="">— identique —</option>${opt(R.ARTICLES.filter(a => a.serie > 0), a => a.id, a => a.libelle, pr.articleEntretienId || '')}</select></div></div>
  ...
  Object.assign(R.actions, {
    ...
    userRole(el) { const u = R.userById(el.dataset.id); u.role = el.value; R.journal(`Accès modifié — ${u.nom} → ${R.ROLES[u.role].label}`); R.touch(); R.toast('Accès mis à jour', 'good'); },
    userToggle(el) { const u = R.userById(el.dataset.id); u.actif = !u.actif; R.journal(`Code ${u.actif ? 'réactivé' : 'désactivé'} — ${u.nom}`); R.touch(); R.render(); }
  });

  /* ===================== PARAMÈTRES ===================== */
  R.pages.parametres = {
    render() {
      const s = S.settings; const w = R.can('equipe', 'w');
