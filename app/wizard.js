...
(function (R) {
  ...
  const calcCures = () => { const w = W(); if (!w.protocoleId) return []; return R.genererCures(protoTemp(), w.dateDebut, +w.d.poids || 70, w.horizon).map(c => c.voie === 'IV' ? Object.assign(c, { heure: w.heure, fauteuil: +w.fauteuil }) : c); };
  ...
  /* ---------- Étapes ---------- */
  function step1(w) {
    const d = w.d; const montrealAide = d.pathologie === 'MC' ? 'A1 < 17 ans · A2 17–40 · A3 > 40 | L1 iléale · L2 colique · L3 iléo-colique · L4 haute | B1 inflammatoire · B2 sténosante · B3 pénétrante · p périnéale' : 'E1 rectite · E2 colite gauche · E3 pancolite | S0 rémission · S1 légère · S2 modérée · S3 sévère';
    return `<div class="grid c21"><section class="card"><div class="card-head"><h2>Identité</h2></div><div class="card-body form-grid">
      ${field('Nom', 'nom', 'text', { attrs: 'required style="text-transform:uppercase"' })}${field('Prénom', 'prenom', 'text', { attrs: 'required' })}${field('IPP', 'ipp', 'text', { attrs: 'placeholder="2026-02601" required' })}
      ...
      ${field('Poids (kg)', 'poids', 'number', { attrs: 'step="0.1" required' })}${field('Taille (cm)', 'taille', 'number')}<div class="field"><label>Médecin référent</label><select data-change="wBind" data-k="medecinId">${opt(medecins(), u => u.id, u => R.userName(u.id), d.medecinId)}</select></div>
    </div></section>
    <section class="card"><div class="card-head"><h2>Maladie</h2></div><div class="card-body form-grid">
      <div class="field span3"><label>Pathologie</label><select data-change="wBindR" data-k="pathologie">${opt(Object.keys(R.PATHOS), k => k, k => R.PATHOS[k].label, d.pathologie)}</select></div>
      ${field('Classification de Montréal', 'montreal', 'text', { span: 3, attrs: `placeholder="${d.pathologie === 'MC' ? 'ex. A2 L3 B1p' : 'ex. E3 S2'}"`, hint: montrealAide })}
      ${field('Date du diagnostic', 'dateDiag', 'date', { span: 3 })}
    </div></section></div>
    <section class="card"><div class="card-head"><h2>Antécédents et traitements</h2></div><div class="card-body form-grid">
      ${field('Traitements associés', 'traitementsAssocies', 'text', { attrs: 'placeholder="ex. azathioprine 150 mg/j, mésalazine 4 g/j"' })}${field('Allergies', 'allergies', 'text', { attrs: 'placeholder="ex. pénicilline (urticaire)"' })}${field('Biothérapies antérieures', 'antecedentsBio', 'text', { attrs: 'placeholder="ex. infliximab 2019–2024, perte de réponse (ADA+)"' })}
      ${field('Comorbidités / facteurs de risque', 'comorbidites', 'text', { span: 3, attrs: 'placeholder="ex. tabagisme actif, antécédent de zona, insuffisance cardiaque (contre-indication anti-TNF si NYHA III–IV)"' })}
    </div></section>`;
  }
  function step2(w) {
    ...
    return `<section class="card"><div class="card-head"><div><h2>Bilan pré-thérapeutique</h2><div class="sub">check-list GETAID 2021 / ECCO 2025 — à compléter avant la première administration</div></div><div class="row"><div class="meter" style="width:140px"><i class="${fait === app.length ? 'good' : 'warn'}" style="width:${app.length ? (fait / app.length * 100).toFixed(0) : 0}%"></i></div><span class="num small">${fait}/${app.length}</span><button type="button" class="btn sm" data-action="wBilanTout">Tout marquer « fait — normal »</button></div></div>
      ...
      <div class="card-foot">Le dépistage de la tuberculose (IGRA + radiographie) et de l’hépatite B conditionne la première cure : une infection tuberculeuse latente se traite ≥ 3 semaines avant le début. Les vaccins vivants sont contre-indiqués sous traitement : à administrer ≥ 3–4 semaines avant.</div></section>`;
  }
  function step3(w) {
    ...
    ${pr ? `<div class="grid c21"><section class="card"><div class="card-head"><div><h2>Cycles de traitement — ${esc(pr.dci)}</h2><div class="sub">doses recalculées sur ${w.d.poids || '?'} kg · arrondi au flacon de ${art ? art.unite + ' ' + art.uniteLib : '—'}</div></div></div><div class="card-body">
      ...
      <section class="card"><div class="card-head"><div><h2>Calendrier prévisionnel</h2><div class="sub">${cures.length} cures · dernière le ${R.fmtDate(cures[cures.length - 1]?.datePrevue)}</div></div></div><div class="card-body flush tbl-wrap"><table class="tbl"><thead><tr><th>N°</th><th>Phase</th><th>Date</th><th>Dose</th><th class="right">Fl.</th></tr></thead><tbody>${cures.map(c => `<tr><td class="mono">${c.n}</td><td>${c.phase === 'Induction' ? R.badge('info', c.label) : `<span class="tag">${c.label}</span>`} <span class="xs muted">${c.voie}</span></td><td class="nowrap">${R.fmtDate(c.datePrevue, { weekday: 'short', day: '2-digit', month: '2-digit', year: '2-digit' })}</td><td class="small dose">${esc(c.doseTexte)}</td><td class="right num">${c.flacons || '—'}</td></tr>`).join('')}</tbody></table></div><div class="card-foot">Les dates tombant un week-end sont décalées au lundi. Chaque cure IV réserve un fauteuil à l’heure indiquée.</div></section></div>` : '<div class="callout info">Sélectionnez un protocole pour construire les cycles.</div>'}`;
  }
  function step4(w) {
    const cats = [...new Set(R.SURVEILLANCE.map(s => s.cat))];
    return `<div class="grid c21"><section class="card"><div class="card-head"><div><h2>Plan de surveillance</h2><div class="sub">proposition issue du protocole — ajustez les éléments et les échéances</div></div></div><div class="card-body stack">${cats.map(cat => `<div><div class="caps mb8">${esc(cat)}</div><div class="stack" style="gap:8px">${R.SURVEILLANCE.filter(s => s.cat === cat).map(s => { const c = w.surv[s.id] || { on: false }; return `<div class="check${c.on ? ' on' : ''}" style="flex-wrap:wrap"><input type="checkbox" id="sv-${s.id}" data-change="wSurvToggle" data-id="${s.id}"${c.on ? ' checked' : ''}><label for="sv-${s.id}" class="grow" style="cursor:pointer"><b>${esc(s.label)}</b><span>${s.mode === 'cure' ? 'à chaque cure' : s.mode === 'periodique' ? 'périodique' : 'échéances datées'}${s.cible ? ' · cible ' + esc(s.cible) : ''}</span></label>
          ${c.on && s.mode === 'echeances' ? `<div class="row" style="width:100%;padding-left:24px">${c.jours.map((j, k) => `<span class="row" style="gap:4px"><span class="tag">${R.libelleJour(j)}</span><input type="date" class="inline-input w150" value="${R.addDays(w.dateDebut, j)}" data-change="wSurvDate" data-id="${s.id}" data-j="${k}"></span>`).join('')}<button type="button" class="btn sm ghost" data-action="wSurvAddDate" data-id="${s.id}">+ échéance</button></div>` : ''}
          ${c.on && s.mode === 'periodique' ? `<div class="row" style="width:100%;padding-left:24px"><span class="small">tous les</span><input type="number" class="inline-input w70" value="${c.tousLes}" data-change="wSurvPeriod" data-id="${s.id}"><span class="small">jours (${Math.floor(w.horizon / c.tousLes)} échéances sur l’horizon)</span></div>` : ''}</div>`; }).join('')}</div></div>`).join('')}</div></section>
    <section class="card"><div class="card-head"><h2>Éléments personnalisés</h2></div><div class="card-body stack">${w.survCustom.map((c, i) => `<div class="row between subtle"><div><b>${esc(c.label)}</b><div class="small muted">${R.fmtDateLong(c.date)}</div></div><button type="button" class="btn sm ghost" data-action="wSurvCustomDel" data-i="${i}">${R.icon('x')}</button></div>`).join('') || '<span class="small muted">Aucun élément ajouté</span>'}
      <div class="form-grid"><div class="field span2"><label>Libellé</label><input type="text" id="w-custom-label" placeholder="ex. IRM pelvienne (fistule), consultation stomathérapie, ostéodensitométrie"></div><div class="field"><label>Échéance</label><input type="date" id="w-custom-date" value="${R.addDays(w.dateDebut, 90)}"></div></div><button type="button" class="btn sm" data-action="wSurvCustomAdd">Ajouter</button>
  ...
  Object.assign(R.actions, {
    ...
    wSurvToggle(el) { const w = W(); const s = R.surv(el.dataset.id); w.surv[s.id] = w.surv[s.id] || { on: false, jours: s.jours ? [...s.jours] : null, tousLes: s.tousLes || null }; w.surv[s.id].on = el.checked; R.render(); },
    wSurvDate(el) { const w = W(); w.surv[el.dataset.id].jours[+el.dataset.j] = R.diffDays(w.dateDebut, el.value); },
    wSurvAddDate(el) { const w = W(); const j = w.surv[el.dataset.id].jours; j.push((j[j.length - 1] || 0) + 182); R.render(); },
  ...
  R.carnetHTML = function (p) {
      ...
      <div class="carnet-page">${band}
        ...
        <dl class="doc-kv"><div><dt>Nom, prénom</dt><dd>${esc(p.nom)} ${esc(p.prenom)}</dd></div><div><dt>Né(e) le</dt><dd>${R.fmtDate(p.ddn)} (${R.age(p.ddn)} ans) · ${p.sexe === 'F' ? 'F' : 'M'}</dd></div><div><dt>IPP</dt><dd>${esc(p.ipp)}</dd></div><div><dt>Téléphone</dt><dd>${esc(p.tel || '')}</dd></div><div><dt>Poids / taille</dt><dd>${p.poids} kg · ${p.taille || '—'} cm</dd></div><div><dt>Allergies</dt><dd>${esc(p.allergies)}</dd></div><div><dt>Maladie</dt><dd>${esc(R.PATHOS[p.pathologie]?.label)} · Montréal ${esc(p.montreal)}</dd></div><div><dt>Diagnostic</dt><dd>${R.fmtDate(p.dateDiag, { month: 'long', year: 'numeric' })}</dd></div></dl>
        <div class="doc-h2">Traitement</div>
        <dl class="doc-kv"><div><dt>Biothérapie</dt><dd>${esc(pr.dci)} <span class="ui" style="font-weight:400">(${esc(pr.specialites)})</span></dd></div><div><dt>Classe</dt><dd>${esc(pr.classe)}</dd></div><div><dt>Voie</dt><dd>${esc(pr.voie)}</dd></div><div><dt>Présentation</dt><dd>${art ? esc(art.libelle) : '—'}</dd></div><div><dt>Schéma</dt><dd>${(p.protocoleSnapshot?.induction || pr.induction).map(e => e.label).join(' · ')} puis ${esc((p.protocoleSnapshot?.entretien || pr.entretien)?.label || '')}</dd></div><div><dt>Posologie actuelle</dt><dd>${esc(cureCourante?.doseTexte || '')}</dd></div><div><dt>Début du traitement</dt><dd>${R.fmtDate(p.dateDebut)}</dd></div><div><dt>Prémédication</dt><dd>${esc(p.premedication || (p.cures.find(c => c.premedication && c.premedication !== 'Aucune') ? p.cures.filter(c => c.premedication && c.premedication !== 'Aucune').slice(-1)[0].premedication : 'Aucune'))}</dd></div><div><dt>Traitements associés</dt><dd>${esc(p.traitementsAssocies)}</dd></div><div><dt>Biothérapies antérieures</dt><dd>${esc(p.antecedentsBio)}</dd></div><div><dt>Médecin référent</dt><dd>${esc(R.userName(p.medecinId))}</dd></div><div><dt>Chef de service</dt><dd>${esc(s.chef)}</dd></div></dl>
        ...
        <p class="ui" style="font-size:9pt;margin:8px 0 0"><b>Vaccinations :</b> ${esc(vacc && vacc.commentaire ? vacc.commentaire : 'à compléter')} — vaccins vivants atténués contre-indiqués pendant le traitement et 3 mois après l’arrêt ; grippe chaque année, pneumocoque, hépatite B, HPV, zona recombinant recommandés.</p>
        ${foot(1)}</div>

      <div class="carnet-page">${band}
        <div class="doc-h2" style="margin-top:0">Tableau des cures — ${esc(pr.dci)}</div>
      ...
      <div class="carnet-page">${band}
        ...
        ${ech.map(x => `<tr class="${x.statut === 'faite' ? 'doc-done' : ''}"><td>${esc(x.label)}${x.cible ? `<br><span style="font-size:8pt;color:#7E8C88">Cible : ${esc(x.cible)}</span>` : ''}</td><td class="m">${R.fmtDate(x.echeance)} (${R.libelleJour(x.jour || 0)})</td><td class="m">${x.dateFaite ? R.fmtDate(x.dateFaite) : ''}</td><td style="font-size:9pt">${esc(x.resultat || '')}</td><td></td></tr>`).join('')}
        ...
        <ul class="doc-list ui" style="font-size:9.5pt">${p.surveillance.filter(x => x.mode === 'cure').map(x => `<li>${esc(x.label)}</li>`).join('')}<li>Poids, température, tension artérielle et pouls avant l’administration ; recherche d’une infection en cours (fièvre, toux, brûlures urinaires, plaie) ; grossesse éventuelle.</li>${pr.voie.includes('IV') ? `<li>Perfusion : ${esc(pr.dureePerfusion)}. ${esc(pr.surveillancePerf)}</li>` : ''}</ul>
      ...
      <div class="carnet-page">${band}
        ...
        <div class="doc-cols ui" style="font-size:9.5pt">
          <div><div class="doc-box alert"><b>Contactez rapidement l’hôpital de jour si :</b><ul class="doc-list"><li>fièvre ≥ 38 °C, frissons, fatigue inhabituelle ;</li><li>toux persistante, essoufflement, sueurs nocturnes ;</li><li>infection urinaire, cutanée, dentaire, ORL ou herpès / zona ;</li><li>diarrhée avec sang inhabituelle ou douleurs abdominales importantes ;</li><li>éruption cutanée, démangeaisons, gonflement du visage, gêne respiratoire pendant ou après l’administration ;</li><li>jaunisse, urines foncées ;</li><li>fourmillements, troubles de la vue, faiblesse d’un membre ;</li><li>ganglions, amaigrissement, lésion cutanée qui change.</li></ul></div>
            ...
            <div class="doc-box"><b>Vaccinations</b><br>Vaccins vivants atténués (BCG, ROR, fièvre jaune, varicelle, zona vivant) <b>contre-indiqués</b> pendant le traitement et 3 mois après l’arrêt. Recommandés : grippe chaque année, pneumocoque, hépatite B, HPV, zona recombinant, dTP-coq. L’entourage peut être vacciné normalement.</div></div>
