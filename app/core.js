...
(function (R) {
  ...
  R.prochainCreneau = (date, duree, heureSouhaitee, exclude, extra) => {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      for (const m of mins) { const v = R.verifierCreneau(d, R.toH(m), duree, exclude, extra); if (v.ok) return { date: d, heure: R.toH(m), fauteuil: v.fauteuil, decale: i > 0 }; }
    }
    return null;
  };
  ...
  R.reserverCures = (cures, heureSouhaitee, extra) => {
    ...
    cures.forEach(c => { delete c._reserv; });
    return cures;
  };
  ...
  function rendre() {
    ...
    app.innerHTML = `<div class="shell">
      <aside class="rail" id="rail">
        ...
        <nav class="nav"><div style="height:8px"></div>${NAV.filter(i => R.can(i.mod, i.w ? 'w' : 'r')).map(i => `<button class="nav-item${(page === i.id || (i.id === 'patients' && (page === 'patient' || page === 'carnet' || page === 'compteRendu' || page === 'impression'))) ? ' active' : ''}" data-go="${i.id}">${R.icon(i.icon)}<span>${i.label}</span>${i.id === 'dashboard' && nCrit ? `<span class="count">${nCrit}</span>` : ''}</button>`).join('')}</nav>
  ...
  R.statsPeriode = (debut, fin) => {
    ...
    const capacite = joursOuverts * R.hdj().maxParJour; const passees = cures.filter(x => { const d = x.c.dateReelle || x.c.datePrevue; return d >= debut && d <= (fin < t ? fin : t) && ['realisee', 'manquee', 'prevue'].includes(x.c.statut); }).length;
