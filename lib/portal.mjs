...
import { resolveToken, resolveStatsToken } from './clips.mjs';
import { computeCyclePayouts } from './payouts.mjs';

export async function clipperPortal(token, { kind = 'submission' } = {}) {
  const resolved = kind === 'stats' ? await resolveStatsToken(token) : await resolveToken(token);
  if (!resolved) return null;
  const { cycle, clipperId, clipperName } = resolved;

  // Facebook is the manager's private month-end tracking — clippers never see
  // those clips or their views on their links, and no total counts them.
  const { rows: clips } = await query(
    `select id, platform, url, status, views, likes, comments, engagement, flags,
       ...
       from clips
      where cycle_id = $1 and clipper_id = $2 and platform <> 'facebook'
  ...
  const { facebook: _fb, ...byPlatform } = mine?.byPlatform || {};
  return {
    cycle,
    clipperName,
    clips,
