...
/** Calibratable rate: cents per 1,000 paid checks, stored in app_settings so
 * the estimate can be tuned to match the real Apify bill. */
export async function getPaidCheckRate() {
  const { rows } = await query(`select apify_cents_per_1k from app_settings where id = 1`);
  return Number(rows[0]?.apify_cents_per_1k) || 420;
}
...
export async function getSpending(month) {
  ...
  const rate = await getPaidCheckRate();
  const costForChecks = (n) => estimateScanCostCents({ paidPosts: n, perThousand: rate });
  ...
  const [paidDaily, ytDaily, payoutDaily, perCampaign, totals, storage] = await Promise.all([
    ...
    query(
      `select vh.checked_at::date as day, count(*)::int as n
        ...
        where c.platform in ('tiktok','instagram','facebook')
    ...
    query(
      `select ca.id, ca.name, ca.avatar_url,
              ...
              coalesce((select count(*) from view_history vh
                ...
                where cy4.campaign_id = ca.id and c.platform in ('tiktok','instagram','facebook')
              ...
              coalesce((select count(*) from view_history vh
                ...
                where cy5.campaign_id = ca.id and c.platform in ('tiktok','instagram','facebook')), 0)::int as paid_checks_all
         from campaigns ca
    ...
    query(
      `select
         (select count(*) from view_history vh join clips c on c.id = vh.clip_id
           where c.platform in ('tiktok','instagram','facebook')) as paid_checks_all,
         (select count(*) from view_history vh join clips c on c.id = vh.clip_id
