...
export async function addClip({ cycle, clipperId, url, addedVia = 'manual', confirmDuplicate = false }) {
  ...
  // IG/YouTube URLs don't reveal the poster, so manually-added clips whose
  // stat fetch fails sit with a NULL handle — invisible to @handle searches.
  // Until the first successful fetch resolves the real owner, default to the
  // clipper's ONLY linked account on this platform (unambiguous by definition).
  let accountHandle = handle;
  if (!accountHandle) {
    const { rows: accs } = await query(
      `select handle from clipper_accounts where clipper_id = $1 and platform = $2 limit 2`,
      [clipperId, platform],
    );
    if (accs.length === 1) accountHandle = accs[0].handle;
  }
  ...
  try {
    const { rows } = await query(
      `insert into clips (cycle_id, clipper_id, platform, url, normalized_key, account_handle, added_via, flags)
         ...
         returning *`,
      [cycle.id, clipperId, platform, canonicalUrl || url, key, accountHandle, addedVia, flags],
    );
