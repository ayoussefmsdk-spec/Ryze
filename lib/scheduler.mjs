...
async function runDueChecksInner() {
  ...
  for (const { id } of cycles) {
    ...
    const fb = schedule.fb || { mode: 'manual' }; // FB defaults to manual-only

    const groupDue = async (g, platforms, defMinutes) => {
      if (g.mode === 'interval') return intervalDue(await lastChecked(id, platforms), Number(g.everyMinutes) || defMinutes);
      if (g.mode === 'daily') return dailyDue(await lastChecked(id, platforms), g.atLocal, tz);
      return false; // 'manual' -> never due automatically
    };
    const freeDue = await groupDue(free, ['youtube'], 360);
    const paidDue = await groupDue(paid, ['tiktok', 'instagram'], 1440);
    const fbDue = await groupDue(fb, ['facebook'], 1440);

    // Each group runs on ITS OWN schedule — 'paid' never drags Facebook along
    // and vice versa. 'all' is reserved for the manual button.
    if (freeDue) { await runCheck(id, { scope: 'free' }); ran.push({ cycleId: id, action: 'check-free' }); }
    if (paidDue) { await runCheck(id, { scope: 'paid' }); ran.push({ cycleId: id, action: 'check-paid' }); }
    if (fbDue) { await runCheck(id, { scope: 'facebook' }); ran.push({ cycleId: id, action: 'check-facebook' }); }
  }
