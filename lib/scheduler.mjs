...
/** Daily mode: due when a scheduled local time has passed and no check ran today (cycle tz). */
function dailyDue(last, atLocal, tz) {
  const now = timeInTz(tz);
  const today = todayInTz(tz);
  const lastDay = last
    ? new Intl.DateTimeFormat('en-CA', { timeZone: tz || 'UTC', year: 'numeric', month: '2-digit', day: '2-digit' }).format(last)
    : null;
  if (lastDay === today) return false; // already ran today
  return (atLocal || ['06:00']).some((t) => now >= t);
}
