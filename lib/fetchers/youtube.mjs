...
export async function fetchChannelRecent(handle, n = 20) {
  ...
  // The playlist API pages at 50 — follow pageToken so "last 100 posts"
  // really means 100, not a silent cap at 50.
  const ids = [];
  let pageToken = '';
  while (ids.length < n) {
    const plRes = await fetch(
      `https://www.googleapis.com/youtube/v3/playlistItems?part=contentDetails&maxResults=${Math.min(50, n - ids.length)}&playlistId=${uploads}${pageToken ? `&pageToken=${pageToken}` : ''}&key=${key}`,
      { signal: AbortSignal.timeout(20000) },
    );
    if (!plRes.ok) break;
    const plData = await plRes.json();
    ids.push(...(plData.items || []).map((it) => it.contentDetails?.videoId).filter(Boolean));
