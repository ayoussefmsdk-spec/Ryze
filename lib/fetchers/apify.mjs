...
export async function fetchInstagramStats(urls) {
  ...
  const suspectKeys = new Set(
    [...out.entries()]
      .filter(([, s]) => s.likes != null && s.views < s.likes)
      .map(([k]) => k),
  );
  if (suspectKeys.size) {
    const suspectUrls = urls.filter((u) => suspectKeys.has(parseClip(u).key));
    try {
      collect(await runActorSync(INSTA_ACTOR, {
        directUrls: suspectUrls,
        resultsType: 'details',
        resultsLimit: suspectUrls.length,
        addParentData: false,
      }));
    } catch { /* keep first-pass numbers — the ig_suspect flag still raises */ }
  }
  return out;
}
