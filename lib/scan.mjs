...
import { scanFacebookPage } from './fetchers/facebook.mjs';

const PAID = new Set(['tiktok', 'instagram', 'facebook']);
...
export async function scanClipper({ cycleId, clipperId, perAccount = 20, autoApprove = false, onlyAccounts = null, scanHashtags = undefined }) {
  ...
  for (const acc of accounts) {
    ...
    try {
      ...
      else if (acc.platform === 'facebook') posts = await scanFacebookPage(acc.handle, perAccount);
      else {
    ...
    if (posts.warning) issues.push({ account: accKey, note: `partial scan — ${posts.warning}` });
    if (posts.length === 0) {
  ...
  // Same post already tracked under ANOTHER campaign (same clippers work for
  // several clients) — those get their own section in the results instead of
  // blending in, labeled with where they already live.
  const candKeys = [...new Set(candidates.map((c) => c.key))];
  const otherCycleKeys = new Map(
    candKeys.length
      ? (await query(
          `select distinct on (c.normalized_key)
                  c.normalized_key as key, ca.name as campaign_name, cy.name as cycle_name
             from clips c
             join cycles cy on cy.id = c.cycle_id
             join campaigns ca on ca.id = cy.campaign_id
            where cy.id <> $1 and c.normalized_key = any($2)
            order by c.normalized_key, c.created_at desc`,
          [cycleId, candKeys],
        )).rows.map((r) => [r.key, `${r.campaign_name} · ${r.cycle_name}`])
      : [],
  );
  ...
  // FREE stat refresh: a re-scan sees clips that are already in the cycle
  // ("duplicate") WITH their freshly fetched numbers — apply them instead of
  // throwing them away. This is what makes "scan once at month-end" work for
  // scan-only platforms like Facebook: the same scan that finds new posts also
  // brings every known post's views up to date, at no extra cost.
  // Only upward (fetched >= stored): protects manual floors and never lets a
  // degraded scrape drag numbers down.
  let refreshed = 0;
  for (const r of reject) {
    if (r.reason !== 'duplicate' || !r.key || !r.stat) continue;
    const s = r.stat;
    ...
    const { rows } = await query(
      `update clips set
          ...
          engagement = $4,
          caption = coalesce(caption, $5),
          posted_at = coalesce(posted_at, $6),
          thumbnail_url = coalesce(thumbnail_url, $7),
          last_checked_at = now()
        where cycle_id = $8 and normalized_key = $9
          and status <> 'rejected' and views <= $1
        returning id`,
      [views, s.likes ?? null, s.comments ?? null,
       computeEngagement({ views, likes: s.likes, comments: s.comments }),
       s.caption || null, r.postedAt || null, s.thumbnailUrl || null,
       cycleId, r.key],
    );
    if (rows[0]) {
      refreshed++;
      await query(
        ...
        [rows[0].id, views, s.likes ?? null, s.comments ?? null],
      );
    }
  }
  ...
  const skipped = reject
    ...
    .map((r) => ({
      reason: r.reason,
      elsewhere: r.elsewhere || null, // 'Campaign · Cycle' for in_other_campaign
      url: r.url,
      platform: r.platform,
...
export async function ingestScannedClip({ cycleId, clipperId, candidate, autoApprove = false }) {
  ...
  const reasonFlag = ['outside_dates', 'missing_hashtag', 'in_other_campaign'].includes(s.reason) ? [s.reason] : [];
  const { rows } = await query(
    `update clips set
