...
import { apifyHealth } from './system.mjs';

export async function getDashboard() {
  const [counts, activeCycles, paidRow, topClippers, platformRows, recentClips, flaggedClips, healthRow] = await Promise.all([
    query(`
      select
    ...
    `),
    query(`
      select c.id, c.platform, c.url, c.views, c.flags, c.last_checked_at, c.created_at,
             cl.name as clipper_name, cy.id as cycle_id, ca.name as campaign_name
        from clips c
        join clippers cl on cl.id = c.clipper_id
        join cycles cy on cy.id = c.cycle_id
        join campaigns ca on ca.id = cy.campaign_id
       where array_length(c.flags, 1) > 0 and c.status <> 'rejected'
       order by coalesce(c.last_checked_at, c.created_at) desc
       limit 8
    `),
    query(`select apify_ok_at, apify_error_at, apify_error from app_settings where id = 1`),
  ]);

  // Total owed = each active cycle's computed payouts minus what's already
  // been paid toward it (partial payments count).
  const paidPerCycle = new Map(
    (await query(`select cycle_id, coalesce(sum(amount_cents),0)::bigint as paid from payouts group by cycle_id`))
      .rows.map((r) => [r.cycle_id, Number(r.paid)]),
  );
  let totalViews = 0;
  let owedCents = 0;
  const cycleCards = [];
  for (const cy of activeCycles.rows) {
    ...
    owedCents += Math.max(0, total - (paidPerCycle.get(cy.id) || 0));
    cycleCards.push({
      ...cy,
      computedCents: total,
  ...
  return {
    ...
    flaggedClips: flaggedClips.rows,
    apify: apifyHealth(healthRow.rows[0]),
  };
}
