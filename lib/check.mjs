...
import { fetchFacebookStats } from './fetchers/facebook.mjs';
...
const APIFY_PLATFORMS = new Set(['tiktok', 'instagram', 'facebook']);
...
async function applyStats(clip, cycle, stats) {
  ...
  if (
    ...
    && !clip.manual_override // manual numbers are governed by the floor rule below
  ...
  // IG/FB safety: a manually verified number is a FLOOR. Their scraped view
  // counts are unreliable (plays vs views metrics) — when the manager typed
  // the real number, only a HIGHER fetched count may replace it, never lower.
  const keepManual = ['instagram', 'facebook'].includes(clip.platform) && clip.manual_override
    && Number(clip.views) > 0 && stats.views < Number(clip.views);
...
/** FB clips whose own account_handle is unresolved borrow the CLIPPER's linked
 *  Facebook page — that's what powers the page-route fetch for /reel/ links
 *  (whose URLs never reveal the posting page). */
async function withFbHandles(clips) {
  // Candidates per clip = its own stored handle PLUS every facebook page
  // linked to its clipper — a stale/wrong handle on the clip (or a page that
  // was re-linked later) must never block the page-route fetch.
  const clipperIds = [...new Set(clips.map((c) => c.clipper_id))];
  const linked = new Map(); // clipper_id -> [handles]
  if (clipperIds.length) {
    const { rows } = await query(
      `select clipper_id, handle from clipper_accounts where platform = 'facebook' and clipper_id = any($1)`,
      [clipperIds],
    );
    for (const r of rows) linked.set(r.clipper_id, [...(linked.get(r.clipper_id) || []), r.handle]);
  }
  return clips.map((c) => {
    const handles = [...new Set([c.account_handle, ...(linked.get(c.clipper_id) || [])].filter(Boolean))];
    return { url: c.url, handle: handles[0] || null, handles };
  });
}
...
async function recordPaidMiss(clip) {
  // Facebook misses never escalate to "removed": its page-route fetch only
  // sees a page's recent posts, so an older clip missing from a batch is
  // expected — not evidence of deletion.
  if (clip.platform === 'facebook') {
...
async function paidBudgetLeft() {
  ...
  const { rows } = await query(
    `select count(*)::int as used
      ...
      where c.platform in ('tiktok','instagram','facebook') and vh.checked_at::date = current_date`,
  );
  const used = rows[0]?.used ?? 0;
  return { left: Math.max(0, cap - used), cap, used };
}

/**
 * runCheck(cycleId, { scope }) — scope: 'free' (YouTube only) | 'paid'
 * (TikTok+IG) | 'facebook' (FB only) | 'all' (YouTube+TikTok+IG — never FB).
...
export async function runCheck(cycleId, { scope = 'all' } = {}) {
  ...
  // ---- YouTube (free; runs for 'all' and 'free' scopes) ---------------------
  const ytClips = clips.filter((c) => c.platform === 'youtube');
  if (ytClips.length && (scope === 'all' || scope === 'free')) {
    try {
  ...
  // ---- TikTok + Instagram + Facebook (paid, per-scope) ----------------------
  // scope: 'all' = YouTube+TikTok+IG · 'paid' = TikTok+IG only (their
  // schedule) · 'facebook' = FB only (its own button/schedule) · 'free' = YT.
  // Facebook is deliberately NOT part of 'all': it's month-end-only tracking,
  // and 'all' gets pressed casually — FB spends Apify credit only when its
  // dedicated scope runs.
  const doPaid = scope === 'all' || scope === 'paid';
  const doFb = scope === 'facebook';
  let capInfo = null; // set when the daily spend cap limited this run
  if (doPaid || doFb) {
    const guard = await paidBudgetLeft();
    let budget = guard.left;
    for (const [platform, fetcher, enabled] of [
      ['tiktok', fetchTikTokStats, doPaid],
      ['instagram', fetchInstagramStats, doPaid],
      ['facebook', fetchFacebookStats, doFb],
    ]) {
      const group = clips.filter((c) => c.platform === platform);
      if (!group.length) continue;
      if (!enabled) { skipped += group.length; continue; }
      // Spend cap: check as many as the budget allows — STALEST first, so
      // every clip still rotates through across runs — and say so, instead of
      // silently skipping the whole platform.
      let toCheck = group;
      if (budget < group.length) {
        capInfo = { cap: guard.cap, used: guard.used };
        toCheck = [...group]
          .sort((a, b) => new Date(a.last_checked_at || 0) - new Date(b.last_checked_at || 0))
          .slice(0, Math.max(0, budget));
        skipped += group.length - toCheck.length;
        if (!toCheck.length) continue;
      }
      budget -= toCheck.length;
      try {
        // FB gets url+handle pairs (its fetcher can fall back to the page route).
        const stats = platform === 'facebook'
          ? await fetcher(await withFbHandles(toCheck))
          : await fetcher(toCheck.map((c) => c.url));
        for (const clip of toCheck) {
        ...
        for (const clip of toCheck) await markFailed(clip.id, 'fetch_failed');
        failed += toCheck.length;
      }
    }
  } else {
    skipped += clips.filter((c) => APIFY_PLATFORMS.has(c.platform)).length;
  }
  ...
  return { checked, failed, skipped, frozen: dueFreeze, dupesFlagged, capInfo };
}
...
export async function fetchSingleClip(clipId) {
  ...
  if (APIFY_PLATFORMS.has(clip.platform)) {
    if ((await paidBudgetLeft()).left < 1) throw new Error('Daily paid-check cap reached — raise it in System');
    const fetcher = clip.platform === 'tiktok' ? fetchTikTokStats
      : clip.platform === 'instagram' ? fetchInstagramStats
        : fetchFacebookStats;
    const fbPairs = clip.platform === 'facebook' ? await withFbHandles([clip]) : null;
    const stats = fbPairs ? await fetcher(fbPairs) : await fetcher([clip.url]);
    const s = stats.get(clip.normalized_key);
    if (s) return applyStats(clip, cycle, s);
    // FB dead-end: without a page handle the page-route can't even try —
    // say exactly what unblocks it instead of failing silently.
    if (clip.platform === 'facebook' && !fbPairs?.[0]?.handle) {
      throw new Error(
        'Couldn’t fetch this Facebook post: its URL doesn’t reveal the posting page, and this clipper has no Facebook page linked. Link their page on the Clippers page (platform: facebook), then recheck — the check finds the post through the page.',
      );
    }
