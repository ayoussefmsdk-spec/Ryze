...
async function withFbHandles(clips) {
  const need = [...new Set(clips.filter((c) => !c.account_handle).map((c) => c.clipper_id))];
  ...
  if (need.length) {
    ...
    for (const r of rows) if (!fallback.has(r.clipper_id)) fallback.set(r.clipper_id, r.handle);
  }
  return clips.map((c) => ({ url: c.url, handle: c.account_handle || fallback.get(c.clipper_id) || null }));
}
