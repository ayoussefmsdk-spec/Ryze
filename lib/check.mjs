    ...
    for (const [platform, fetcher, enabled] of [
      ...
      try {
        ...
        const stats = platform === 'facebook'
          ? await fetcher(toCheck.map((c) => ({ url: c.url, handle: c.account_handle })))
  ...
  if (APIFY_PLATFORMS.has(clip.platform)) {
    ...
    const fetcher = clip.platform === 'tiktok' ? fetchTikTokStats
      : clip.platform === 'instagram' ? fetchInstagramStats
        : fetchFacebookStats;
    const stats = clip.platform === 'facebook'
      ? await fetcher([{ url: clip.url, handle: clip.account_handle }])
      : await fetcher([clip.url]);
    const s = stats.get(clip.normalized_key);
    ...
    if (clip.platform === 'facebook' && !clip.account_handle) {
      throw new Error(
