...
async function applyStats(clip, cycle, stats) {
  ...
  // IG safety: a manually verified number is a FLOOR. IG's scraped view count
  // is unreliable (plays vs views metrics) — when the manager typed the real
  // number, only a HIGHER fetched count may replace it, never a lower one.
  const keepManual = clip.platform === 'instagram' && clip.manual_override
