...
const FETCH_FLAGS = ['unknown_account', 'outside_dates', 'missing_hashtag', 'ig_suspect', 'view_drop', 'removed', 'fetch_failed'];
    ...
    for (const [platform, fetcher] of [
      ...
      try {
        ...
        for (const clip of group) {
          ...
          else { await markFailed(clip.id, 'fetch_failed'); failed++; }
        }
      } catch {
