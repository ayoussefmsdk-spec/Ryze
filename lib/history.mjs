 ...
 */
export async function clipsPostedPerDay({ cycleId = null, clipperId = null, excludeFacebook = false } = {}) {
  ...
  if (excludeFacebook) wheres.push(`c.platform <> 'facebook'`);
  const { rows } = await query(
 ...
 *  excludeFacebook: the clipper-facing stats link never counts FB views. */
export async function clipperDailySeries(clipperId, { excludeFacebook = false } = {}) {
  const { rows } = await query(
    `select day, sum(mx)::bigint as total from (
        select vh.clip_id, vh.checked_at::date as day, max(vh.views) as mx
          from view_history vh
         ...
         where c.clipper_id = $1 and c.status = 'approved'
           and ($2 = false or c.platform <> 'facebook')
         group by vh.clip_id, vh.checked_at::date
     ) t group by day order by day`,
    [clipperId, excludeFacebook],
  );
  return rows.map((r) => ({ label: String(r.day), value: Number(r.total) }));
}
